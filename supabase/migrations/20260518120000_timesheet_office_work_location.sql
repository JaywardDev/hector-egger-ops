alter table public.timesheet_preferences
  drop constraint if exists timesheet_preferences_preferred_work_mode_check;

alter table public.timesheet_preferences
  add constraint timesheet_preferences_preferred_work_mode_check
  check (preferred_work_mode in ('factory', 'site', 'office', 'mixed'));

alter table public.timesheet_entries
  drop constraint if exists timesheet_entries_work_mode_check;

alter table public.timesheet_entries
  add constraint timesheet_entries_work_mode_check
  check (work_mode in ('factory', 'site', 'office', 'mixed'));

alter table public.timesheet_entry_activities
  drop constraint if exists timesheet_entry_activities_work_mode_check;

alter table public.timesheet_entry_activities
  add constraint timesheet_entry_activities_work_mode_check
  check (work_mode in ('factory', 'site', 'office'));
