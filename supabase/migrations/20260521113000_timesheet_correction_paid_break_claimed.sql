...
begin
  ...
  if p_is_public_holiday is null or p_unpaid_break is null or p_paid_break is null then raise exception 'Break and public holiday values are required.'; end if;
  ...
  if not p_is_public_holiday then
    ...
    if abs(v_calculated_allocation - round((p_payable_hours - case when p_paid_break then 0.5 else 0 end)::numeric, 1)) > 0.01 then raise exception 'Allocation must equal attendance span minus claimed paid break and unpaid break before submitting.'; end if;
    
    if abs(p_allocation_hours - v_calculated_allocation) > 0.01 then raise exception 'Allocation hours do not match activity and leave values.'; end if;
  end if;
  ...
  update public.timesheet_entries set status='submitted', submitted_at=timezone('utc', now()), time_in=case when p_is_public_holiday then null else p_time_in end, time_out=case when p_is_public_holiday then null else p_time_out end, work_mode=p_work_mode, leave_type=case when p_is_public_holiday then null else p_leave_type end, leave_hours=case when p_is_public_holiday then 0 else p_leave_hours end, is_public_holiday=p_is_public_holiday, unpaid_break=case when p_is_public_holiday then false else p_unpaid_break end, paid_break=case when p_is_public_holiday then false else p_paid_break end, payable_hours=p_payable_hours, allocation_hours=p_allocation_hours, approved_at=null, approved_by_profile_id=null, returned_at=null, returned_by_profile_id=null, return_comment=null where id=v_entry.id;
