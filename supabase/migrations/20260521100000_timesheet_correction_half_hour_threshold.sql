...
declare
  ...
  v_derived_paid_break boolean := false;
begin
  ...
  if p_is_public_holiday is null or p_unpaid_break is null then raise exception 'Break and public holiday values are required.'; end if;
  ...
  if not p_is_public_holiday then
    ...
    if mod(extract(minute from p_time_in)::int, 30) <> 0 or mod(extract(minute from p_time_out)::int, 30) <> 0 then
      raise exception 'Time in and time out must be on 30-minute boundaries.';
    end if;
    ...
    -- Current derived-entitlement model: paid break starts from 3.0h attendance; 2.5h does not qualify.
    v_derived_paid_break := round((extract(epoch from (p_time_out - p_time_in)) / 3600)::numeric, 1) >= 3.0;
    ...
    v_calculated_allocation := round((v_activity_total + p_leave_hours + case when v_derived_paid_break then 0.5 else 0 end)::numeric, 1);
    if abs(v_calculated_allocation - p_allocation_hours) > 0.01 then raise exception 'Allocation hours do not match activity, leave, and paid break values.'; end if;
    if abs(p_allocation_hours - p_payable_hours) > 0.01 then raise exception 'Allocation must equal payable total before submitting.'; end if;
  end if;
  ...
  update public.timesheet_entries set status='submitted', submitted_at=timezone('utc', now()), time_in=case when p_is_public_holiday then null else p_time_in end, time_out=case when p_is_public_holiday then null else p_time_out end, work_mode=p_work_mode, leave_type=case when p_is_public_holiday then null else p_leave_type end, leave_hours=case when p_is_public_holiday then 0 else p_leave_hours end, is_public_holiday=p_is_public_holiday, unpaid_break=case when p_is_public_holiday then false else p_unpaid_break end, paid_break=case when p_is_public_holiday then false else v_derived_paid_break end, payable_hours=p_payable_hours, allocation_hours=p_allocation_hours, approved_at=null, approved_by_profile_id=null, returned_at=null, returned_by_profile_id=null, return_comment=null where id=v_entry.id;
  ...
  if v_inserted_count <> v_expected_count then raise exception 'Failed to replace all activity rows.'; end if;
