...
export const listInventoryItems = async ({ session }: MutationActor): Promise<InventoryItemRecord[]> => {
  ...
  const response = await supabase.request(
    "/rest/v1/inventory_items?select=id,item_code,name,unit,description,created_at,updated_at&order=name.asc",
    {
      cache: "no-store",
...
export const createInventoryItem = async ({
  ...
  const response = await supabase.request(
    "/rest/v1/inventory_items?select=id,item_code,name,unit,description,created_at,updated_at",
    {
      method: "POST",
      headers: {
...
export const updateInventoryItem = async ({
  ...
  const supabase = createServiceRoleSupabaseClient();
  const response = await supabase.request(`/rest/v1/inventory_items?id=eq.${itemId}`, {
    method: "PATCH",
    headers: {
      "Content-Type": "application/json",
      Prefer: "return=minimal",
