...
export const calculatePayableHours = (
  input: Pick<
    SaveTimesheetEntryInput,
    "isPublicHoliday" | "timeIn" | "timeOut" | "unpaidBreak"
  >,
) => {
...
export const calculateAllocationHours = (
  input: Pick<
    SaveTimesheetEntryInput,
    "isPublicHoliday" | "activities" | "leaveHours" | "paidBreak"
  >,
) => {
  if (input.isPublicHoliday) return 8;
  const activityHours = input.activities.reduce(
    (total, row) => total + row.hours,
    0,
  );
  return roundHours(
    activityHours + input.leaveHours + (input.paidBreak ? 0.5 : 0),
  );
};

type IncompleteActivityRow = {
  index: number;
...
export const incompleteActivityMessage = (workMode: TimesheetWorkMode) =>
  workMode === "mixed"
    ? "Complete project, task, and location for every activity row with hours."
    : "Complete project and task for every activity row with hours.";

export const getIncompleteActivityRows = (
  input: Pick<
    SaveTimesheetEntryInput,
    "isPublicHoliday" | "workMode" | "activities"
  >,
): IncompleteActivityRow[] => {
  ...
  return input.activities.flatMap((row, index) => {
    ...
    const missingLocation =
      input.workMode === "mixed" && !activityModes.has(row.workMode);

    return missingProject || missingTask || missingLocation
      ? [{ index, missingProject, missingTask, missingLocation }]
      : [];
  });
};
...
export const validateTimesheetEntryInput = (
  ...
  if (payableHours === null || payableHours < 0) {
    throw new Error(
      "Time in and time out must form a valid same-day time span.",
    );
  }
  ...
  if (
    !Number.isFinite(input.leaveHours) ||
    input.leaveHours < 0 ||
    input.leaveHours > 24
  ) {
  ...
  if (incompleteActivityRows.length > 0) {
    throw new Error(incompleteActivityMessage(input.workMode));
  }

  const activities = input.activities.filter((row) => row.hours > 0);
  for (const row of activities) {
    if (!validProjectIds.has(row.projectId))
      throw new Error("Select a valid project for each activity row.");
    if (!validTaskIds.has(row.taskId))
      throw new Error("Select a valid task for each activity row.");
  ...
  return {
    ...
    leaveHours: roundHours(input.leaveHours),
  };
};
