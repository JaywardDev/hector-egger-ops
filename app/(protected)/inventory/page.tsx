    ...
    <section className="space-y-4 text-sm text-zinc-700">
      ...
      {canWrite ? (
        <div className="space-y-2 rounded-md border border-zinc-200 bg-white p-3">
          ...
          <InventoryItemForm action={createInventoryItemAction} materialGroups={materialGroups} />
        </div>
      ) : (
        ...
        <ul className="space-y-3">
          {items.map((item) => (
            <li key={item.id} className="rounded-md border border-zinc-200 bg-white p-3">
              {canWrite ? (
                <InventoryItemForm action={updateInventoryItemAction} materialGroups={materialGroups} item={item} />
              ) : (
