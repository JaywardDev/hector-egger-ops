...
export async function createProductionEntryFormAction(formData: FormData) {
  ...
  // Return to a fresh new-entry form (rather than the detail page) so operators
  // can log entries back-to-back. The created id is passed so the page can show a
  // success notice with a link and remount the form to clear it.
  if (createdEntryId) redirect(`/production/entries/new?success=${encodeURIComponent("Entry created.")}&created=${createdEntryId}`);
}
...
export async function updateProductionReasonFormAction(formData: FormData) { const { session, roles } = await requireProtectedAccess("/production/reasons"); if (!hasReasonAdminRole(roles)) throw new Error("Admin access is required for production reason management"); const kind = String(formData.get("kind") ?? ""); const reasonId = normalizeUuid(formData.get("reason_id")); if (!reasonId) return toMessage("/production/reasons", "Reason id is required.", "error"); const labelRaw = formData.get("label"); const label = labelRaw === null ? undefined : String(labelRaw).trim(); if (labelRaw !== null && !label) return toMessage("/production/reasons", "Reason label is required.", "error"); const isActiveRaw = formData.get("is_active"); const input = { label: label || undefined, isActive: isActiveRaw === null ? undefined : isActiveRaw === "true" }; if (kind === "downtime") await updateProductionDowntimeReason({ session, accessContext: { accountStatus: "approved", roles }, reasonId, input }); else await updateProductionInterruptionReason({ session, accessContext: { accountStatus: "approved", roles }, reasonId, input }); revalidatePath("/production/reasons"); revalidatePath("/production/entries/new"); toMessage("/production/reasons", "Reason updated."); }
