...
export function ProductionEntryForm({ formAction, submitLabel, operators, canAssignOtherOperator, projectFiles, downtimeReasons, interruptionReasons, latestTimeRemainingEndByProjectFile = {}, initialValues }: Props) {
  ...
  const [timeRemainingStart, setTimeRemainingStart] = useState(String(initialTimeRemainingStart ?? "")); const [timeRemainingEnd, setTimeRemainingEnd] = useState(String(initialValues?.timeRemainingEndMinutes ?? 0));
  ...
  return <form action={formAction} className="grid gap-3 sm:grid-cols-2">
    ...
    <FormField label="Project File" htmlFor="project_file_id"><Select id="project_file_id" name="project_file_id" value={projectFileId} onChange={(event) => { const nextProjectFileId = event.currentTarget.value; setProjectFileId(nextProjectFileId); if (!timeRemainingStartTouched) setTimeRemainingStart(nextProjectFileId ? String(latestTimeRemainingEndByProjectFile[nextProjectFileId] ?? "") : ""); }} required><option value="">Select project file</option>{activeProjectFiles.map((project) => <option key={project.id} value={project.id}>{project.project_name} — {project.project_file}{project.project_sequence === null ? "" : ` / Sequence ${project.project_sequence}`}</option>)}</Select></FormField>
    ...
    <FormField label="Time Remaining Start" htmlFor="time_remaining_start_minutes"><Input id="time_remaining_start_minutes" name="time_remaining_start_minutes" type="number" min={0} value={timeRemainingStart} onChange={(event) => { setTimeRemainingStartTouched(true); setTimeRemainingStart(event.currentTarget.value); }} required /></FormField>
