...
export default async function StockTakePage({ searchParams }: StockTakePageProps) {
  ...
  return (
    <section className="space-y-4 text-sm text-zinc-700">
      <div>
        <h2 className="text-base font-semibold text-zinc-900">Stock take sessions</h2>
        <p className="text-zinc-600">Create location-based count sessions and open them for operational quantity capture.</p>
      </div>
      ...
      {canCreateSessions ? (
        <form action={createStockTakeSessionAction} className="space-y-2 rounded-md border border-zinc-200 bg-white p-3">
          <h3 className="font-medium text-zinc-900">Create session</h3>
          <div className="grid gap-2 md:grid-cols-2">
            <input name="title" placeholder="Session title" required className="rounded-md border border-zinc-300 px-2 py-1.5" />
            <select name="stockLocationId" required defaultValue="" className="rounded-md border border-zinc-300 px-2 py-1.5">
      ...
      {stockTakeSessions.length === 0 ? (
        <p className="rounded-md border border-zinc-200 bg-white px-3 py-3">No stock take sessions yet.</p>
      ) : (
        <ul className="space-y-3">
          {stockTakeSessions.map((stockTakeSession) => (
            <li key={stockTakeSession.id} className="rounded-md border border-zinc-200 bg-white p-3">
              <div className="flex flex-col gap-2 md:flex-row md:items-start md:justify-between">
                <div className="space-y-1">
                  ...
                  <p>Status: {stockTakeSession.status}</p>
