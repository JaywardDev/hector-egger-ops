...
import { createStockTakeSession, upsertStockTakeEntry } from "@/src/lib/stock-take/sessions";
...
const normalizeRequired = (value: FormDataEntryValue | null) => String(value ?? "").trim();
...
const toStockTakeDetailMessage = (sessionId: string, message: string, type: "success" | "error") =>
...
export async function createStockTakeSessionAction(formData: FormData) {
  ...
  try {
    ...
    redirect(`/stock-take/${createdSession.id}?success=${encodeURIComponent("Stock take session created.")}`);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Could not create stock take session.";
...
export async function saveStockTakeEntryAction(formData: FormData) {
  ...
  if (!sessionId || !inventoryItemId || Number.isNaN(countedQuantity) || countedQuantity < 0) {
    toStockTakeDetailMessage(sessionId, "Item and counted quantity are required.", "error");
  }

  const { session } = await requireProtectedAccess();
  ...
  try {
    ...
    const message = error instanceof Error ? error.message : "Could not save counted quantity.";
