...
function TimesheetDayRow({
  ...
  return (
    <OperationalListRow
      ...
      className="grid-cols-[minmax(0,1fr)_auto] items-start gap-x-3 gap-y-1 py-2 sm:py-3 sm:grid-cols-[minmax(0,1fr)_auto_auto] sm:items-center sm:gap-3"
      density="dense"
...
export function WeeklyTimesheetView({
  ...
  return (
    <section className="rounded-[1.35rem] border border-zinc-200/80 bg-white px-3 py-3 shadow-[0_18px_45px_rgba(15,23,42,0.04)] sm:px-4 sm:py-4" aria-labelledby="weekly-timesheet-heading">
      ...
      <div className="mt-2 sm:divide-y sm:divide-zinc-100">
        {days.map((day, index) => (
          <div key={day.date}>
            <TimesheetDayRow
              day={day}
